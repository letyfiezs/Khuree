...
export function AdminUsers({ initial }: { initial: AdminUserItem[] }) {
  ...
  async function resetPassword() {
    ...
    if (passwordUser.credentialKind === "pin" && !/^\d{4}$/.test(newPassword)) return setError("Шинэ PIN яг 4 оронтой байна.");
    if (passwordUser.credentialKind !== "pin" && newPassword.length < 8) return setError("Шинэ нууц үг дор хаяж 8 тэмдэгттэй байна.");
  ...
  return <>
    ...
    {passwordUser && <div className="device-modal-backdrop access-modal-backdrop"><section className="device-modal access-modal password-reset-modal"><button className="access-modal-close" aria-label="Хаах" onClick={() => setPasswordUser(undefined)}>×</button><p className="section-kicker">ХЭРЭГЛЭГЧИЙН НЭВТРЭЛТ</p><h2>{passwordUser.credentialKind === "pin" ? "PIN шинэчлэх" : "Нууц үг шинэчлэх"}</h2><p>Энэ хэрэглэгчид шинэ {passwordUser.credentialKind === "pin" ? "4 оронтой PIN" : "нууц үг"} тохируулна.</p><label>Шинэ {passwordUser.credentialKind === "pin" ? "PIN" : "нууц үг"}<input type="password" inputMode={passwordUser.credentialKind === "pin" ? "numeric" : undefined} autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(passwordUser.credentialKind === "pin" ? event.target.value.replace(/\D/g, "").slice(0, 4) : event.target.value)} /></label><label>Шинэ {passwordUser.credentialKind === "pin" ? "PIN" : "нууц үгээ"} давтах<input type="password" inputMode={passwordUser.credentialKind === "pin" ? "numeric" : undefined} autoComplete="new-password" value={repeatPassword} onChange={(event) => setRepeatPassword(passwordUser.credentialKind === "pin" ? event.target.value.replace(/\D/g, "").slice(0, 4) : event.target.value)} /></label><div className="device-modal-actions"><button disabled={saving === passwordUser.id} onClick={() => setPasswordUser(undefined)}>Цуцлах</button><button className="primary-button" disabled={saving === passwordUser.id} onClick={() => void resetPassword()}>{saving === passwordUser.id ? "Шинэчилж байна…" : "Шинэчлэх"}</button></div></section></div>}
