...
export function RecentlyWatched() {
  ...
  return <section className="catalog home-catalog recently-watched">
    ...
    <div className="film-rail">{items.slice(0, 12).map((item) => <Link href={`/watch/${encodeURIComponent(item.slug)}`} className="film-card" key={item.id}>
      <div className={`poster ${item.posterUrl ? "has-poster" : ""}`} style={{ background: item.posterUrl ? `linear-gradient(0deg,#000b,transparent 52%),url(${item.posterUrl}) center/cover no-repeat` : "linear-gradient(155deg,#4b0b12,#080808)" }}>
        <span className="poster-mark">ХҮРЭЭ</span><span className="film-tag">ҮРГЭЛЖЛҮҮЛЭХ</span>{!item.posterUrl && <span className="poster-title">{item.seriesTitle || item.title}</span>}<span className="play-chip">▶</span>
      </div>
