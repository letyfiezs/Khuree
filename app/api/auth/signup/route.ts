...
export async function POST(request: Request) {
  ...
  if (!body.name?.trim() || !body.email?.includes("@") || !body.password || body.password.length < 8) return Response.json({ error: "Нэр, зөв имэйл болон дор хаяж 8 тэмдэгттэй нууц үг оруулна уу." }, { status: 400 });
